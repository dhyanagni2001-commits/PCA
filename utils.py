...
def get_projections(word_seq, words, E, w):
    ...
    for wd in word_seq:
        emb = find_embedding(wd, words, E)
        proj_seq.append(np.dot(emb, w_norm))
...
def comparative_projections(word_seq, words, E, wd1, wd2):
    ...
    # ii) Direction vector w = w1 - w2
    w = w1 - w2
    w /= np.linalg.norm(w)

    # iii) Return projections onto w
...
def find_analog(wd1, wd2, wd3, words, E):
    ...
    # ii) Analogy vector (normalize)
    w = w2 - w1 + w3
    w = w / np.linalg.norm(w)

    # iii) Positions (must use np.where, not .index)
    id1 = np.where(words == wd1)[0][0]
    id2 = np.where(words == wd2)[0][0]
    id3 = np.where(words == wd3)[0][0]

    # iv) Most similar word to the analogy vector
    wd4 = find_most_sim_word_w(w, words, E, [id1, id2, id3])
...
def similarity(wd1, wd2, words, E):
    ...
    # ii) Cosine similarity (rows already unit-normalized)
    sim = np.dot(w1, w2)
